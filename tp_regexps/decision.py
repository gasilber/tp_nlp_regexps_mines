...
class Decision(object):
    ...
    @staticmethod
    def __get_header(data: str):
        m = regexps.header_re.search(data)
        if m:
            return m.group("header")
        return None

    @staticmethod
    def __get_chambre(data: str):
        m = regexps.chambre_re.search(data)
        if m:
            return m.group("chambre")
        return None
